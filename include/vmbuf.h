...
inline void vmbuf_make(struct vmbuf *vmb) { memset(vmb, 0, sizeof(struct vmbuf)); }
inline void vmbuf_reset(struct vmbuf *vmb) { vmb->read_loc = vmb->write_loc = 0; }
...
inline char *vmbuf_data(struct vmbuf *vmb) { return vmb->buf; }
inline char *vmbuf_data_ofs(struct vmbuf *vmb, size_t loc) { return vmb->buf + loc; }

inline size_t vmbuf_wavail(struct vmbuf *vmb) { return vmb->capacity - vmb->write_loc; }
inline size_t vmbuf_ravail(struct vmbuf *vmb) { return vmb->write_loc - vmb->read_loc; }

inline char *vmbuf_wloc(struct vmbuf *vmb) { return vmb->buf + vmb->write_loc; }
inline char *vmbuf_rloc(struct vmbuf *vmb) { return vmb->buf + vmb->read_loc; }

inline size_t vmbuf_rlocpos(struct vmbuf *vmb) { return vmb->read_loc; }
inline size_t vmbuf_wlocpos(struct vmbuf *vmb) { return vmb->write_loc; }

inline void vmbuf_rlocset(struct vmbuf *vmb, size_t ofs) { vmb->read_loc = ofs; }
inline void vmbuf_wlocset(struct vmbuf *vmb, size_t ofs) { vmb->write_loc = ofs; }

inline void vmbuf_rrewind(struct vmbuf *vmb, size_t by) { vmb->read_loc -= by; }
inline void vmbuf_wrewind(struct vmbuf *vmb, size_t by) { vmb->write_loc -= by; }

inline size_t vmbuf_capacity(struct vmbuf *vmb) { return vmb->capacity; }

inline void vmbuf_unsafe_wseek(struct vmbuf *vmb, size_t by) { vmb->write_loc += by; }
inline int vmbuf_wseek(struct vmbuf *vmb, size_t by) { vmbuf_unsafe_wseek(vmb, by); return vmbuf_resize_if_full(vmb); }
inline void vmbuf_rseek(struct vmbuf *vmb, size_t by) { vmb->read_loc += by; }

inline void vmbuf_rreset(struct vmbuf *vmb) { vmb->read_loc = 0; }
inline void vmbuf_wreset(struct vmbuf *vmb) { vmb->write_loc = 0; }
...
inline void vmbuf_memset(struct vmbuf *vmb, int c, size_t n) { memset(vmbuf_data(vmb), c, n); }
